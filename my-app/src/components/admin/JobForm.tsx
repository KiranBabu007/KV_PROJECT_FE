...
const JobForm: React.FC<JobFormProps> = ({ onCancel, job, mode = 'create', onSubmit }) => {
  ...
  useEffect(() => {
    if (job && mode === 'edit') {
      ...
      const salaryNumber = typeof job.salary === 'string' ?
                           parseInt(job.salary.replace(/[^0-9]/g, ''), 10) || 0 :
                           job.salary;

      const experienceNumber = typeof job.experience === 'string' ?
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="salary"
            type="number"
            value={formData.salary / 100000} // Display as LPA
            onChange={(e) => setFormData({ ...formData, salary: Number(e.target.value) * 100000 })} // Store as full amount
            required
          />
        </div>
        <div className="space-y-2">
          <Label htmlFor="experience">Experience (Years)</Label>
          <Input id="experience" type="number" min="0" value={formData.experience} onChange={(e) => setFormData({ ...formData, experience: Number(e.target.value) })} required />
        </div>
        <div className="space-y-2">
          <Label htmlFor="numOfPositions">Number of Positions</Label>
          <Input id="numOfPositions" type="number" min="1" value={formData.numOfPositions} onChange={(e) => setFormData({ ...formData, numOfPositions: Number(e.target.value) })} required />
        </div>
        <div className="space-y-2">
          <Label htmlFor="bonusForReferral">Referral Bonus</Label>
          <Input id="bonusForReferral" type="number" min="0" value={formData.bonusForReferral} onChange={(e) => setFormData({ ...formData, bonusForReferral: Number(e.target.value) })} required />
        </div>
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <div className="flex space-x-2">
          <Input placeholder="Add a requirement" value={newRequirement} onChange={(e) => setNewRequirement(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addRequirement())}/>
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {requirements.map((requirement) => (
            <Badge key={requirement} variant="secondary" className="pr-1">
              {requirement}
              <button type="button" onClick={() => removeRequirement(requirement)} className="ml-1 hover:bg-gray-200 rounded">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
